...
- **开发环境**：所有前后端代码修改**必须在 `develop` 分支**进行
- **推送 `develop` 后**：AI 必须主动询问是否要合并到 `main` 分支
- **每次部署前**：无论自动还是手动部署，**必须询问用户确认**后才能执行
- 合并到 `main` 后才会触发生产环境的部署

此规则已记录在 `.cursor/rules/branch-workflow.mdc` 中，作为 AI 持久化指导。
