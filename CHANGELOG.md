...
- AI 需求分析报告新增结构化审阅摘要，将质量评分、待确认问题、低质量输入提醒、测试策略和自动化准备清单聚合到审阅区。
- AI 需求分析输入来源扩展为“上传文档 / 直接输入 / 历史恢复”，并将模板与模型相关入口收敛到高级设置。
- AI 需求分析运行视图新增解析耗时、TTFT、总分析耗时指标，并补充移动端 E2E 验收与前端覆盖率门禁脚本。
- AI 需求分析交互主线升级为“选择输入 / 输入质检 / AI 分析运行 / 结构化审阅 / 生成用例”五阶段，并在左侧增加输入质检摘要、右侧强化分析运行与结果分区。

### Fixed

- AI 需求分析修订流程改为仅展示最新 AI 修订版报告，避免初版与修订版拼接后干扰最终审阅。
- AI 需求分析五阶段与运行阶段进度条补齐完成/运行/失败/等待图标语义，最终“生成用例”阶段可作为可点击入口。
- AI 需求分析结构化审阅补齐一键测试策略和 Agent 执行准备兜底推导，模型漏字段时不再只显示“待补充/待识别”。
- AI 需求分析通过后底部区域移除重复“需求分析已通过”文案，仅保留清空日志与重置分析，并缩小报告页底部运行日志抽屉高度。
- AI 流式接口稳定性增强：服务端 SSE 每次写入后显式 flush，并对流式响应增加 `no-transform`，降低反代/负载均衡空闲超时或缓冲导致的中断概率。
- AI 需求分析流式中断提示修正，按当前端点显示 `/api/ai/analyze/stream`，不再误提示 `/api/ai/generate/stream`。
- AI 需求分析“生成用例”兜底带入完整报告，避免分析报告标题不固定时生成页主输入为空导致最后一步不可生成。
- AI 需求分析报告内 Mermaid 流程图下载修复：SVG 下载前补齐命名空间与尺寸，PNG 转换改用 data URL 并统一可靠下载触发。
- AI 需求分析流式输出一旦产生报告正文，右侧自动切换到“报告”视图，避免用户停留在过程日志里错过结果。
- 后端文件服务测试补齐后台解析 worker 所需 Prisma mock，消除 `recoverStaleParsingFiles` 相关测试日志噪音。
- AI 需求分析刷新/重新进入时会尝试恢复最近的流式记录快照；PDF 导出流程图 PNG 渲染失败时明确提示使用 Mermaid 源码块兜底。

## 2026-06-22

### Added

- AI 需求分析新增“上传文档 / 直接输入”双入口和四阶段流程条，纯文本需求可跳过文件解析直接进入分析。
