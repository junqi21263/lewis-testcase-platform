...
| 后端连 compose 里的 Postgres 容器 | **`DATABASE_URL` 主机名用容器名**（如 `testcase_postgres`）时，后端须与该容器在同一 Docker 网络。在 CNB 变量设置 **`CNB_BACKEND_EXTRA_NETWORKS`**（逗号分隔网络名，无空格），部署成功后脚本会对后端执行 **`docker network connect`**。示例：`lewis_testcase_platform_testcase_network` |
