...
test.describe('E2E: AI 需求分析全流程', () => {
  ...
  test('完整流程：上传文档 → AI 分析 → 审阅 → 通过', async ({ page }) => {
    ...
    await expect(page.getByTestId('ai-analysis-log-status-success')).toHaveCount(2)
    await expect(page.getByTestId('ai-analysis-log-status-success').first()).toHaveAttribute(
      'data-appearance',
      'plain-symbol',
    )
    await expect(page.getByTestId('ai-analysis-log-status-success').first()).not.toHaveClass(/rounded-full|border/)
